...
def render_p1_observation_markdown(report: P1ObservationReport) -> str:
    ...
    lines = [
        ...
        "## 报告状态与 P0 影子门禁",
        "",
        f"- Report Status: `{_md(report.report_status.value)}`",
        f"- Run ID: `{_md(report.run_id)}`",
        f"- P0 Gate: `{_md(overview.p0_gate_overall or '-')}`（`{_md(overview.p0_gate_mode or '-')}`）",
        f"- P0 Integrity: `{_md(overview.p0_integrity_status or '-')}`",
        "- P1 报告状态只表示观察数据完整性，不是门禁结论，也不会修改 Jenkins 结果。",
        "",
        "## 数据源健康度",
        "",
        _md_table(
            ("数据源", "期望", "状态", "版本", "问题", "Artifact"),
            [
                (
                    item.source_name,
        ...
        "",
        (
            f"- Workload operations: {overview.workload_operation_count}；"
            f"success={overview.operation_success_count}，failed={overview.operation_failed_count}，"
            f"timeout={overview.operation_timeout_count}。"
        ),
    ]
    ...
    if report.usage_coverage is None:
        ...
        lines.extend(
            [
                f"- complete={usage.complete_count}，partial={usage.partial_count}，missing={usage.missing_count}，not_applicable={usage.not_applicable_count}",
                _md_table(
                    ("资源", "已知总量", "样本量", "缺失", "完整性"),
                    [
    ...
    if report.flaky is None:
        ...
        lines.extend(
            [
                f"- newly suspected={len(flaky.newly_suspected)}，newly confirmed={len(flaky.newly_confirmed)}，ongoing confirmed={len(flaky.ongoing_confirmed)}，stale={flaky.stale_count}",
                _flaky_table(
                ...
                "## 隔离、恢复与超期治理",
                "",
                "QUARANTINED 是治理标签，不代表测试通过，也不会自动跳过用例。",
                "",
                f"- quarantined={len(flaky.quarantined)}，recovering={len(flaky.recovering)}，recovered={len(flaky.recovered)}，overdue={len(flaky.overdue)}",
                _flaky_table(
                ...
                "",
                _md_table(
                    ("迁移 ID", "状态", "触发", "原因", "样本", "Actor", "证据"),
                    [
                        (
                            item.transition_id,
                            f"{item.from_state.value if item.from_state else '-'} -> {item.to_state.value}",
    ...
    lines.append(
        ...
        else "本次没有需要额外处理的 attention item。"
    )
    lines.extend(
        [
            "",
            "## 完整性与证据入口",
            "",
            f"- Required source failures: {report.integrity.required_source_failure_count}",
            f"- Issue codes: `{_md(', '.join(report.integrity.issue_codes) or '-')}`",
            "- 完整机器数据请查看 `p1-observation.json`；指标与 Flaky 详情请回到各自源 Artifact。",
            "",
            _md_table(
                ("展示窗口", "总数", "已展示", "已省略", "完整源"),
                [
...
def _dimension_text(value: dict[str, str | None]) -> str:
    return ", ".join(f"{key}={item if item is not None else '-'}" for key, item in value.items()) or "run"
...
def _flaky_table(values: tuple[FlakyStateSummary, ...]) -> str:
    return _md_table(
        ("Case", "环境/画像", "当前/检测状态", "样本", "投影", "Owner", "到期"),
        [
            (
                item.case_id,
                f"{item.environment}/{item.execution_profile}",
                f"{item.current_state.value}/{item.detected_state.value}",
                item.sample_size,

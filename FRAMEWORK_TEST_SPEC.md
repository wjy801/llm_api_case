...
本文档描述当前已实现框架能力的测试规范、用例写法和本地验收口径。当前 CI 尚未接入，因此所有质量检查暂以本地命令执行结果为准。
...
当前 CI 没有接入，不能把质量门禁、指标聚合、Flaky 历史治理写成已实现能力。
...
- 成功率、状态码分布、耗时分位线、重试率聚合。
- SSE 首 token 延迟、chunk 间隔、总耗时。
- 异步任务排队、执行、完成耗时。
- Flaky 标记、隔离、历史失败率和连续失败次数。
- 可被 CI 读取的 JSON 汇总和质量门禁失败状态。

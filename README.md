...
- 模型账单以模型响应中的 request ID 查询对应 usage，避免共享钱包的其他延迟结算污染结果。
- 成功请求断言 usage 状态已完成且 `quota_yuan > 0`；失败请求断言自身的 `quota_yuan == 0`。
- usage GET 使用统一的有限重试与轮询策略；不重试可能重复扣费的模型 POST。
- 一个用例并发调用多个模型时，分别保存每个 request ID，并逐条验证对应 usage。

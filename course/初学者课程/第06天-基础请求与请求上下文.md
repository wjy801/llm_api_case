...
第一次打开 `common/base_request.py` 时，不要从第一行连续读到最后一行。这个文件同时连接普通请求、Retry、Polling、Capture 和运行观察，一次展开全部实现会把多个时间粒度混在一起。

本节只追踪“无 Retry、非流式、单次 POST”路径。每次只打开当前问题所需的一小段代码，回答“输入是什么、改变了什么、交给谁、责任到哪里结束”后，再进入下一个函数。代码中的 `...` 表示本课暂不展开的实现，不能把摘录当成可独立运行的完整代码。

### 第一步：业务参数怎样进入 Module Request

从 Day5 的 Task 调用开始，只看这一句：

```python
response = request_client.echo(payload)
```

此时只知道 Task 把 `payload` 交给了 Request，还不知道 HTTP method、path 或真实发送位置。带着这个问题打开 `module/offline_framework_example/request.py` 中的 `echo()`：
...
这段代码完成第一次转换：业务名 `echo` 被翻译为 Echo path，`payload` 被放进 `json` 参数，同时附加运行观察元数据。

到这里能够确认的是“Module Request 表达接口调用合同”。还不能说请求已经发送，因为代码只是继续调用 `self.post(...)`。下一个问题自然变成：`post()` 做了什么？

### 第二步：动词函数只补 method

沿 `self.post(...)` 进入 `common/base_request.py`，只需看两行：
...
`post()` 没有创建 Session、没有合并 Header，也没有执行网络请求。它只固定 `method="POST"`，然后把 path 和其余参数交给统一入口 `request()`。

因此不要停在 `post()` 猜测传输细节。现在需要追问的是：统一入口如何区分框架控制参数与真正要发送的参数？

### 第三步：统一入口先分流参数

进入 `request()` 后先停在开头，不急着向下滚动：
...
三个 `pop()` 表示这些值由框架自己消费，不会原样进入 `Session.request()`。Echo 没有显式传入 Retry 策略，所以 `retry_policy` 为 `None`。

运行元数据在紧接着的观察入口被规范化；Day6 只确认它不会作为业务参数交给 Session，不进入 RuntimeObserver 的内部实现。继续向下，只摘出本次真正经过的无 Retry 分支：
...
这时仍未发送请求。`request()` 先要求 `_build_request_context()` 把分散输入标准化，再把结果交给单次发送路径。`_send_with_retry()` 属于 Day9，本课看到分支存在即可，不提前展开。

### 第四步：Context 在发送前逐项形成

进入 `_build_request_context()` 后也不要一次复制整个函数。先看第一组状态变化：
...
这里依次得到完整 URL、kwargs 工作副本和最终 timeout。阅读 `_build_url()` 时只需确认：绝对 HTTP URL 原样返回，相对 path 与 `config.base_url` 组合。阅读 `_copy_request_kwargs()` 时只需确认：每个顶层值分别尝试 `deepcopy`，复制失败的值会回退为原引用，所以这是常见 payload 隔离，不是绝对隔离保证。

`clamp_timeout()` 在这里出现，是因为 Context 必须保存最终发送值；它如何受 Retry deadline 约束留到 Day9。现在继续看 Header 怎样进入同一份工作副本：
...
本次 Header 先从 kwargs 中取出，再与 Session Header 合并后写回。默认路径下，Session Header 提供基础值，本次 Header 覆盖同名键。由此可以区分：Session 保存跨请求状态，`request_kwargs["headers"]` 保存本次 attempt 的快照。
...
现在 method、path、url 与最终 kwargs 被放入同一个 `RequestContext`。它没有在这里获得 Response，也不拥有 Session；它只是发送前已经解析完成的请求事实。

到这里应先停下来核对：原始 `payload` 位于调用方，复制后的 payload 位于 `context.kwargs["json"]`，长期 Header 位于 Session，本次 Header 位于 `context.kwargs["headers"]`。分清这四个位置后再继续。

### 第五步：单次 Group 包住发送过程

Context 构造完成后，`request()` 把它交给 `_send_single_group()`。本课只看这个函数的骨架：
...
`group.bind(context)` 把本次实际发送 Context 与单次 Request Group 关联。`finally` 表示无论 `_send()` 返回还是抛出异常，Group 都执行自己的结束动作。

这不是网络发送位置，也不等于 Group 关闭 Response。Quality 如何消费观察事件留到后续课程；当前只保留“Group 包住一次发送生命周期”这一事实。

### 第六步：真实发送在 `_send()` 中发生

现在进入 `_send()`，仍按执行顺序分三小段阅读。第一段发生在网络调用之前：

```python
self._run_before_middlewares(context)
```

所有前置 Middleware 接触同一个 Context，可以读取或扩展 attempt 事实。Day7 才展开诊断副本与脱敏，Day8 再展开 Capture；本课只确定它们位于发送前。
...
Session 接收的 method、url 和 kwargs 全部来自当前 Context。若传输抛出异常，异常钩子先获得观察机会，随后原传输异常继续向上抛出；异常钩子自身的问题不能替代原传输失败。

只有 Session 正常返回后，才进入最后两行：
...
这证明“获得 Response”只是传输返回事实。`_send()` 没有在这里把状态码解释成业务成功，Response 还要继续返回给 Task、Test 或其他消费者。

### 第七步：资源退出不在成功返回链中

最后单独看 `close()`，不要把它误接到每次成功请求之后：
...
它关闭的是 BaseRequest 长期持有的 Session。成功路径返回的 Response 已越过 BaseRequest 边界，不能由这两行推出“Response 已经关闭”；Response 的消费与关闭责任在本课后文继续展开。

### 走完以后再汇总主链

只有逐段回答完前面的所有权问题后，才把本次路径压缩为：

```text
echo()
→ post()
→ request()分流控制参数
→ _build_request_context()形成attempt快照
→ _send_single_group()绑定并收尾Group
→ _send()执行Middleware与Session发送
→ Response返回调用方
```

这条链刻意不包含 `_send_with_retry()`、`poll_get()`、`_poll_get_with_policy()`、RuntimeObserver 内部实现和默认 Middleware 细节。它们分别由 Day9、Day10、Day18 和 Day7～8 建立独立模型。

完成导读后，用两个现有测试回查刚才经过的节点：

```powershell
.\.venv\Scripts\python.exe -m pytest tests/test_base_request_middleware.py::TestBaseRequestMiddlewarePipeline::test_runs_middlewares_in_registration_order -q
.\.venv\Scripts\python.exe -m pytest tests/test_base_request_retry_polling.py::test_default_request_does_not_retry -q
```

第一个测试中的 method、url、timeout 与 Header 断言对应 Context 构造，事件顺序对应 `_send()`；第二个测试只证明默认 503 Response 被发送一次并返回，不证明 503 是业务成功。

...
- 不能证明 Allure、JUnit 或 Pipeline Summary 已生成；
...
本图保留前五课的运行入口、收集阶段、两种请求路径、Middleware、Capture 双分支和 pytest teardown 边界。由于本课 Smoke 切片走 Request Client 路径，图中把两种上游路径折叠为一个明确标注的节点，不再画统一的 `Task -> 领域 Request` 实线；本课只展开 Response 之后的 Assertions 与 Schema。
...
flowchart TD
    ...
    subgraph COLLECTION["收集阶段（第二课已掌握）"]
        C1["Case 数据源"]
        C2["loader<br/>校验并构造 Case"]
        C3["pytest.param<br/>稳定参数 ID"]
        C4["精确 nodeid"]
        C1 --> C2
        C2 --> C3
        C3 --> C4

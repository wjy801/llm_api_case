...
```mermaid
flowchart TB
    A["目标路径<br/>+ 选择参数"] -->|"Runner 发起<br/>权威预收集"| B["pytest<br/>collect-only"]
    B -->|"退出码 0<br/>且 nodeid 无重复"| C["权威 Case 集合 P<br/>实现为有序元组"]
    B -->|"退出码非 0<br/>或收集异常"| X["停止<br/>不进入 Case 执行"]
    C -->|"明确 nodeid"| D["后续调度<br/>与执行"]
    C -->|"完整列表<br/>与数量"| E["runner-execution.v1"]
    C -->|"仅传 len(P)<br/>且 Quality 启用"| F["后续完整性参照"]
    D --> G["实际 Case<br/>观察事实"]
    F --> H{"观察数量是否<br/>等于计划数量"}
    G --> H
    H -->|"否"| I["记录<br/>完整性错误"]
    H -->|"是"| J["只通过<br/>数量关卡"]
```

...
| 环节 | 对应章节 | 建议时间 |
| --- | --- | ---: |
| 本课约束、答辩规则与 Case 数据卡 | 第 2～5 节 | 8～10 分钟 |
...
> 学习者已经拥有局部节点，却不能稳定地从一个真实 Case 还原调用、对象和失败关系。
...
参数化和收集钩子会推进到足以形成目标测试项，但 fixture setup 尚未执行，其依赖与运行结果没有被验证。
...
当前 Chat payload 不满足媒体路径条件，所以该 Case 不创建输入下载任务。
...
当前 Chat Case 没有 Polling，也不进入输出 Capture。

### 11.3 为什么总图仍保留 Capture

第 7 课总图是第一周项目图，不是当前 Case 的每条边都为实线。
...
5. common/base_request.py
   post -> request -> _build_request_context -> _send_single_group -> _send
...
```text
pytest 执行 TestResponseBodyValidation 的 chat completions Case。setup_method 创建 SmokeRequest、SmokeTask 和 SmokeAssertions。Test 调用 SmokeTask 的 payload builder 得到 dict；当前模型来自 Task 默认值，这是遗留偏差。
...
BaseRequest.post 进入 request。因为没有 retry_policy，request 构造 RequestContext，然后进入普通单请求组和 _send。_send 先执行 before Middleware，再调用 Session.request；正常返回后执行 after Middleware，并把同一个 Response 沿调用栈返回 Test。

Test 依次验证 200、成功 Schema 和 model 具体值。全部满足后 call 正常结束；任何一项不满足都会抛 AssertionError。最后 teardown_method 关闭 SmokeRequest Session，pytest teardown hook 收口适用资源，pytest 再记录完整用例结果。
```
...
- 把 Capture 画成当前 Chat Case 的固定步骤；
...
图中实线表示当前 Smoke 答辩 Case 的已确认路径；协议切片、Capture 和后续课程接口使用虚线。继承关系单独标注，不能当作函数调用。
...
flowchart TD
    ...
    subgraph CASE["当前 Smoke Case：执行时主链"]
        ...
        POST --> REQ
        REQ -->|"构造"| CTX
        CTX --> SEND --> BEFORE --> HTTP
        HTTP -->|"正常返回"| AFTER --> RESP
        RESP -->|"返回 Test"| ASSERT
        ASSERT -->|"全部满足"| TD --> RESULT
    end
...
必须通过方法解析和源码调用确认，不能把 `SmokeRequest.create_chat_completion()` 凭空插入当前 Case。
...
```text
第一周先建立项目地图，再沿真实 Case 学习业务执行链。当前答辩 Case 是 test_chat_completions_response_body。collect-only 只能证明精确 nodeid 被收集，不执行 fixture setup、setup_method、测试体或 HTTP。
...
BaseRequest.post 进入 request。当前没有 retry_policy，所以构造 RequestContext 后进入普通单请求组。_send 依次运行 before Middleware、Session.request 和 after Middleware。Session 成功后，Response 沿调用栈返回 Test；请求异常则进入 on_exception 并重抛原异常。

Test 对 Response 依次验证状态码、成功 Schema 和 model 业务值。断言成功返回原 Response，失败抛 AssertionError；断言失败不进入 Request on_exception。最后 teardown 关闭 Request Session 并收口适用资源，pytest 记录完整用例结果。

函数调用、继承关系、对象流和失败控制必须分开表达。输入与输出 Capture 是条件分支，当前 Chat Case 都不触发。面对新需求，应把场景输入放 Test，把领域动作放 Task，把端点和 HTTP 语义放 Request，把公共发送能力留在 BaseRequest，把结构与业务判断放 Schema 和 Assertions。下一课才讨论 Retry。
```
...
当前 Smoke 答辩 Case 中，`self.smoke_task.create_chat_completion` 实际解析到哪里？
...
当前答辩 Case 没有配置 Retry。遇到连接超时、429 或 503 时，它会保留真实失败，不自动再次发送。
...
到这里，第 7 课完成。第一周的衡量标准不是记住多少类名，而是能沿真实 Case 找到约束，用源码关系和有限证据解释自己的判断。

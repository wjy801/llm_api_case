#!/usr/bin/env python3
"""Continuously send one stable Anthropic Messages payload for cache triggering."""
...
# Leave empty for current gateways. If an older gateway explicitly requires
# the prompt-caching beta header, set this to "prompt-caching-2024-07-31".
ANTHROPIC_BETA = ""

CACHE_TRIGGER_MESSAGE = "这是缓存触发请求，请只回复：OK"

MAX_TOKENS = 500
STREAM = False

# Generate one new cache prefix per process. It stays unchanged for all
# requests made by this process.
FORCE_NEW_CACHE_WRITE = True
CACHE_RUN_ID = uuid4().hex if FORCE_NEW_CACHE_WRITE else "stable-messages-cache"
CACHE_REFERENCE_REPEAT_COUNT = 32
CACHE_REFERENCE_LINES = (
    "企业知识库接入模型网关前，应统一身份认证、租户隔离和最小权限策略，密钥只允许从"
    "受控环境读取，禁止进入代码、日志、报表或客户端页面，并建立轮换与吊销机制。",
    "检索链路需要明确数据来源、更新时间、访问范围和引用标识，对切分、向量化、召回、"
    "重排及上下文拼接分别验证，防止过期内容、越权内容和无来源结论进入模型输入。",
    "模型网关应集中管理模型映射、超时、限流、重试和降级；重试必须区分可恢复错误与"
    "业务错误，并设置总截止时间，避免重复计费、请求放大和下游服务持续拥塞。",
    "上线前应观测请求成功率、首字节耗时、完整耗时、Token 用量及缓存命中情况，同时"
    "对敏感字段脱敏，并通过离线评测与小流量验证确认答案质量和系统稳定性。",
)

REQUEST_TIMEOUT_SECONDS = 60.0
REQUEST_TIME_OFFSETS = (-1, 0, 1)
IMMEDIATE_WORKER_PREPARE_SECONDS = 1.0
# With scheduling enabled, send the cache-write request this many seconds
# before the earliest formal request.
...
def _require_configuration() -> None:
    ...
    if not CACHE_TRIGGER_MESSAGE.strip():
        raise ValueError("CACHE_TRIGGER_MESSAGE must not be empty")
    if MAX_TOKENS <= 0:
...
def _build_cacheable_system_text() -> str:
...
def _build_payload() -> dict[str, object]:
    # Anthropic Messages uses a top-level system field. cache_control marks a
    # cache breakpoint after the long, stable system content.
    return {
        "model": MODEL_NAME,
        ...
        "system": [
            {
                "type": "text",
                "text": _build_cacheable_system_text(),
                "cache_control": {"type": "ephemeral"},
            }
        ],
        "messages": [
            {"role": "user", "content": CACHE_TRIGGER_MESSAGE},
        ],
        "stream": STREAM,
    }
...
def _build_request_headers() -> dict[str, str]:
    headers = {
        ...
        "User-Agent": "api-case-anthropic-cache-request-runner",
    }
    if ANTHROPIC_BETA:

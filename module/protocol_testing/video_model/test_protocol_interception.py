...
from module.protocol_testing.payloads import build_video_v1_media_generations_payload
...
def protocol_interception_case_params() -> list[pytest.ParameterSet]:
    ...
    if not cases:
        return [
            pytest.param(
                ProtocolInterceptionCase(
                    case_id="unconfigured-video-protocol-interception-case",
                    protocol_path="media_generations",
                    header_protocol="openai",
                    body_protocol="video_media",
                    model_id="your-video-model",
                    expected="block",
                ),
                marks=pytest.mark.skip(reason="请先配置 video_model/protocol_interception.csv"),
                id="unconfigured-video-protocol-interception-case",
            )
        ]
...
def build_protocol_interception_payload(case: ProtocolInterceptionCase) -> dict[str, Any]:
    if case.body_protocol == "video_media":
        return build_video_v1_media_generations_payload(case.model_id)
    raise ValueError(f"Unsupported body protocol: {case.body_protocol!r}")


class TestVideoModelProtocolInterception:
    ...
    def test_video_model_protocol_interception(self, case: ProtocolInterceptionCase):
